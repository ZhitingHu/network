...
public:
  ...
  void randSymDirichlet(float a, vector<float>& output) {
    boost::gamma_distribution<> gamma_dist(a);
    boost::variate_generator<boost::mt19937&, boost::gamma_distribution<> >
      gamma_generator(generator, gamma_dist);
    float total    = 0;
  ...
  // [begin,end)
  size_t randDiscrete(const vector<float>& distrib, size_t begin, size_t end) {
    float totprob  = 0;
  ...
  static void logprobsToRelprobs(vector<float> &distrib, size_t begin, size_t end) {
